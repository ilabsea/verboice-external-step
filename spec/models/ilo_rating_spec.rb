...
RSpec.describe IloRating, :type => :model do
	...
	describe 'create' do
		context 'parse date string to date' do
			it {
				rating = create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 1, step_id: @step.id)
		...
		context 'unique date range' do
			context 'valid' do
				it { expect(create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 1, step_id: @step.id)).to be_kind_of(IloRating) }
			end

			context 'ignore when date is already existed' do
				it {
					expect(create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 1, step_id: @step.id)).to be_kind_of(IloRating)
					expect { create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 2, step_id: @step.id) }.to raise_error
				}
			end
		end
		...
		context 'unique code' do
			it {
				expect(create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 1, step_id: @step.id)).to be_kind_of(IloRating)
					expect { create(:ilo_rating, client_from_date: '03/01/2015', client_to_date: '04/01/2015', code: 1, step_id: @step.id) }.to raise_error
			}
		end
	end
  ...
  describe '.exist?' do
  	before(:each) do
	    @rating = create(:ilo_rating, client_from_date: '01/01/2015', client_to_date: '02/01/2015', code: 1, numbers: ['12999999'], step_id: @step.id)
	  end	
